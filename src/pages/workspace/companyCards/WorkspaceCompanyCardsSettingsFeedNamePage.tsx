...
function WorkspaceCompanyCardsSettingsFeedNamePage({
    ...
    const submit = ({name}: WorkspaceCompanyCardFeedName) => {
        CompanyCards.setWorkspaceCompanyCardFeedName(policyID, workspaceAccountID, feed, name);
        Navigation.goBack(ROUTES.WORKSPACE_COMPANY_CARDS_SETTINGS.getRoute(policyID, feed));
    };

    if (isLoadingOnyxValue(cardFeedsResult)) {
